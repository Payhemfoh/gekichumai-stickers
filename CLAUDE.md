...
- `npm run generate:thumbnails` — regenerate the picker-grid WebP thumbnails (`scripts/generate-thumbnails.mjs`, uses `sharp`); re-run after adding/replacing any character image
...
- `src/characters.json` (~180 KB) is the character database: every character's name, image path, and default subtitle parameters (text, fill/stroke colors, x/y offset, rotation, font size, letter spacing). Character images live in `public/img/<series>/` (e.g. `ongeki/`, `chunithm/`, `maimai/`, plus variants like `ongeki_2`, `maimai_derakkuma`). Adding a character = add the PNG under `public/img/` + an entry in `characters.json`, then run `npm run generate:thumbnails`.
- The picker grid (`Picker.tsx`) loads small WebP thumbnails from `public/img/thumb/<same path>.webp` (generated by `scripts/generate-thumbnails.mjs`), while `Canvas.tsx` and the PNG export keep using the full-resolution original at `/img/<path>`. A missing thumbnail falls back to the full image via the `<img onError>` handler.
